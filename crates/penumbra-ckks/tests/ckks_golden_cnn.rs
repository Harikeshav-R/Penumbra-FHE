...
//! Golden exactness for the Phase-4 CNN under CKKS.
...
#[test]
fn ckks_fhe_matches_quantized_cleartext_cnn() {
    let fx = load_fixture();
    ...
    for (s, input_val) in inputs.iter().enumerate() {
        ...
        println!(
            "[ckks:{}] phase4_cnn sample {s}: max |err| = {max_err:.6e} (declared bound {:.3e})",
            penumbra_ckks::hal_backend_name(),
            bounds::PHASE4_CNN
        );

        assert!(
            max_err <= bounds::PHASE4_CNN,
            "sample {s}: CKKS error {max_err:.6e} exceeds declared bound {:.3e} for phase4_cnn",
            bounds::PHASE4_CNN
        );
