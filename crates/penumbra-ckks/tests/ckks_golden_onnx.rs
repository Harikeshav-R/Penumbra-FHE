...
//! Golden exactness for the Phase-6 ONNX model under CKKS.
...
#[test]
fn ckks_fhe_matches_quantized_cleartext_onnx() {
    let fx = load_fixture();
    ...
    for (s, input_val) in inputs.iter().enumerate() {
        ...
        println!(
            "[ckks:{}] phase6_onnx sample {s}: max |err| = {max_err:.6e} (declared bound {:.3e})",
            penumbra_ckks::hal_backend_name(),
            bounds::PHASE6_ONNX
        );

        assert!(
            max_err <= bounds::PHASE6_ONNX,
            "sample {s}: CKKS error {max_err:.6e} exceeds declared bound {:.3e} for phase6_onnx",
            bounds::PHASE6_ONNX
        );
