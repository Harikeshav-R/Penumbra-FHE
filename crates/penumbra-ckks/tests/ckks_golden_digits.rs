...
//! Golden exactness for the Phase-5 digits model under CKKS.
...
#[test]
fn ckks_fhe_matches_quantized_cleartext_digits() {
    let fx = load_fixture();
    ...
    for (s, input_val) in inputs.iter().enumerate() {
        ...
        println!(
            "[ckks:{}] phase5_digits sample {s}: max |err| = {max_err:.6e} (declared bound {:.3e})",
            penumbra_ckks::hal_backend_name(),
            bounds::PHASE5_DIGITS
        );

        assert!(
            max_err <= bounds::PHASE5_DIGITS,
            "sample {s}: CKKS error {max_err:.6e} exceeds declared bound {:.3e} for phase5_digits",
            bounds::PHASE5_DIGITS
        );
