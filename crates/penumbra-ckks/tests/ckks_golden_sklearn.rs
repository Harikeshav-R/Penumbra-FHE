...
fn ckks_fhe_matches_quantized_cleartext_sklearn() {
    ...
    for (s, input_val) in inputs.iter().enumerate() {
        ...
        println!(
            "[ckks:{}] phase6_sklearn sample {s}: max |err| = {max_err:.6e} (declared bound {:.3e})",
        ...
        assert!(
            ...
            "sample {s}: CKKS error {max_err:.6e} exceeds declared bound {:.3e} for phase6_sklearn",
            bounds::PHASE6_SKLEARN
        );
        ...
        for (i, (&g, &w)) in raw_floats.iter().zip(&want_logits).enumerate() {
            assert_eq!(
                g.round() as i64,
                w,
                "sample {s}, slot {i}: {g} does not round to reference {w}"
            );
        }
        ...
        assert_eq!(
            ...
            "sample {s}: argmax prediction does not match expected label"
        );
    }
}
