...
//! Golden exactness for the Phase-6 scikit-learn linear digit classifier under CKKS.
...
#[test]
fn ckks_fhe_matches_quantized_cleartext_sklearn() {
    let fx = load_fixture();
    ...
    println!(
        "[ckks:{}] phase6_sklearn: max |err| = {max_err:.6e} (declared bound {:.3e})",
        penumbra_ckks::hal_backend_name(),
        bounds::PHASE6_SKLEARN
    );

    assert!(
        max_err <= bounds::PHASE6_SKLEARN,
        "CKKS error {max_err:.6e} exceeds declared bound {:.3e} for phase6_sklearn",
        bounds::PHASE6_SKLEARN
    );

    // Each logit rounds to the reference integer
    for (i, (&g, &w)) in raw_floats.iter().zip(&want_logits).enumerate() {
        assert_eq!(
            g.round() as i64,
            w,
            "slot {i}: {g} does not round to reference {w}"
        );
    }

    // Argmax prediction matches expected label
    let pred_label = raw_floats
        ...
        .map(|(i, _)| i as i64)
        .unwrap();
    assert_eq!(pred_label, want_label);
}
