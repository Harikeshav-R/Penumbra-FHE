...
//! Golden exactness for the Phase-7 faces model under CKKS.
...
#[test]
fn ckks_fhe_matches_quantized_cleartext_faces() {
    let fx = load_fixture();
    ...
    for (s, input_val) in inputs.iter().enumerate() {
        ...
        println!(
            "[ckks:{}] phase7_faces sample {s}: max |err| = {max_err:.6e} (declared bound {:.3e})",
            penumbra_ckks::hal_backend_name(),
            bounds::PHASE7_FACES
        );

        assert!(
            max_err <= bounds::PHASE7_FACES,
            "sample {s}: CKKS error {max_err:.6e} exceeds declared bound {:.3e} for phase7_faces",
            bounds::PHASE7_FACES
        );
