...
fn ckks_fhe_matches_quantized_cleartext_logreg() {
    ...
    println!(
        "[ckks:{}] phase2_logreg: max |err| = {max_err:.6e} (declared bound {:.3e})",
    ...
    assert!(
        ...
        "CKKS error {max_err:.6e} exceeds declared bound {:.3e} for phase2_logreg",
        bounds::PHASE2_LOGREG
    );

    assert_eq!(
        rounded_ints[0], want_label,
        "decrypted label does not round to expected label"
    );
}
