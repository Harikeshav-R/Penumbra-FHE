//! Golden exactness for the Phase-6 ONNX-lowered digit CNN (`AGENTS.md` §1.1) — `#[ignore]` by default.
//!
//! > FHE output must equal the quantized-cleartext output, **bit-for-bit.**
//!
//! This is the FHE bit-for-bit gate for the **ONNX front door**: `examples/mnist/onnx_export.py`
//! trains a CNN, exports it to `digit_cnn.onnx`, then loads it *through* `penumbra.load_onnx` and
//! quantizes it via the unchanged Phase-5 service. The committed `phase6_onnx_fixture.json` IR
//! graph is `Conv2d(stride 2) → Requant(fused ReLU) → Linear` — structurally identical to the
//! hand-built `golden_digits`/`golden_qat` fixtures, proving the front door emits nothing new at
//! the IR layer (no `runtime/` change). This test deserializes it, runs the encrypted forward
//! pass, decrypts the 10 logits, and asserts they (and the argmax) equal the cleartext integer
//! oracle bit-for-bit.
//!
...
//! (`tests/test_onnx_fixture.py`) checks fixture self-consistency on every CI run; this FHE gate
//! is run explicitly:
//!
//! ```text
//! cargo test --release --test golden_onnx -- --ignored --nocapture
//! ```
...
fn load_fixture() -> Value {
    let path = std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../examples/mnist/phase6_onnx_fixture.json");
    ...
    for node in &graph.nodes {
        ...
        let out = match &node.op {
            ...
            OpSpec::Linear { weights, bias, .. } => weights
                ...
                .collect(),
            other => panic!("unexpected op in ONNX fixture: {}", other.op_type()),
        };
...
/// FHE ONNX-lowered digit CNN == quantized-cleartext, bit-for-bit, over the committed batch.
/// `#[ignore]` — minutes per sample; run with `--release -- --ignored`.
#[test]
#[ignore = "minutes per sample (108 PBS); run with: cargo test --release --test golden_onnx -- --ignored"]
fn fhe_matches_quantized_cleartext_onnx() {
    let fx = load_fixture();
