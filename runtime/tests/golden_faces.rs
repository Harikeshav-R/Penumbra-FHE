//! Golden exactness for the Phase-7 ONNX-lowered face CNN (`AGENTS.md` §1.1) — `#[ignore]` by default.
//!
//! > FHE output must equal the quantized-cleartext output, **bit-for-bit.**
//!
//! This is the FHE bit-for-bit gate for the **second use case** (closed-set face recognition) and
//! the phase's abstraction proof: `examples/faces/olivetti_export.py` trains a CNN on the first 8
//! people of the Olivetti faces dataset, exports it to `face_cnn.onnx`, then loads it *through*
//! `penumbra.load_onnx` and quantizes it via the unchanged Phase-5 service. The committed
//! `phase7_faces_fixture.json` IR graph is `Conv2d(stride 4) → Requant(fused ReLU) → Linear` —
//! **byte-for-byte the same op vocabulary** as `golden_onnx`/`golden_digits`, proving faces run on
//! the *unchanged* backend with no new IR op (`PROJECT.md` §4, `AGENTS.md` §1.2). This test file is
//! a use-case example living in `tests/`; it adds **zero** lines to `runtime/src/`. It deserializes
//! the fixture, runs the encrypted forward pass, decrypts the 8 identity logits, and asserts they
//! (and the argmax) equal the cleartext integer oracle bit-for-bit.
//!
//! It is **`#[ignore]`d**: at 8 conv channels × 4×4 positions = 128 requant bootstraps per sample
//! it is minutes per sample, too slow for every-commit CI. The fast Python guard
//! (`tests/test_faces_fixture.py`) checks fixture self-consistency on every CI run; this FHE gate
//! is run explicitly:
//!
//! ```text
//! cargo test --release --test golden_faces -- --ignored --nocapture
//! ```
...
fn load_fixture() -> Value {
    let path = std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../examples/faces/phase7_faces_fixture.json");
...
/// exact arithmetic (the generalized per-channel Requant included), so it is the true oracle.
    ...
    for node in &graph.nodes {
        ...
        let out = match &node.op {
            ...
            OpSpec::Linear { weights, bias, .. } => weights
                ...
                .collect(),
            other => panic!("unexpected op in faces fixture: {}", other.op_type()),
        };
...
/// FHE ONNX-lowered face CNN == quantized-cleartext, bit-for-bit, over the committed batch.
/// `#[ignore]` — minutes per sample; run with `--release -- --ignored`.
#[test]
#[ignore = "minutes per sample (128 PBS); run with: cargo test --release --test golden_faces -- --ignored"]
fn fhe_matches_quantized_cleartext_faces() {
    let fx = load_fixture();
    ...
    check_graph_bit_width_budget(&graph).expect("ONNX-lowered bit-width budget must fit");
