//! Golden exactness for the Phase-5 QAT digit CNN (`AGENTS.md` §1.1) — `#[ignore]` by default.
//!
...
//! `phase5_qat_fixture.json` IR graph is `Conv2d(stride 2) → Requant(fused ReLU) → Linear`;
...
//! (`tests/test_qat_fixture.py`) checks fixture self-consistency on every CI run; this
//! FHE gate is run explicitly:
//!
//! ```text
//! cargo test --release --test golden_qat -- --ignored --nocapture
//! ```
    ...
    for node in &graph.nodes {
        ...
        let out = match &node.op {
            ...
            OpSpec::Linear { weights, bias, .. } => weights
                ...
                .collect(),
            other => panic!("unexpected op in QAT fixture: {}", other.op_type()),
        };
...
/// FHE QAT digit CNN == quantized-cleartext, bit-for-bit, over the committed batch.
/// `#[ignore]` — minutes per sample; run with `--release -- --ignored`.
#[test]
#[ignore = "minutes per sample (108 PBS); run with: cargo test --release --test golden_qat -- --ignored"]
fn fhe_matches_quantized_cleartext_qat() {
    let fx = load_fixture();
    ...
    check_graph_bit_width_budget(&graph).expect("QAT bit-width budget must fit");
