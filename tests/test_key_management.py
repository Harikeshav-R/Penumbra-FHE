...
def test_keyset_save_and_load_roundtrip(tmp_path):
    """A KeySet copies to a new directory and loads back with its metadata intact."""
    ks = KeySet.generate(num_blocks=2, directory=tmp_path / "orig")
...
def test_run_encrypted_mismatched_backend_raises(tmp_path):
    """A KeySet whose backend != the run backend fails loudly (AGENTS.md §1.4)."""
    rng = np.random.default_rng(0)
    model = Model([Linear(weight=rng.normal(size=(3, 8)), bias=rng.normal(size=3))], input_bits=4)
    model.quantize(rng.uniform(0.0, 16.0, size=(64, 8)), n_bits=4)
    assert model.graph is not None
    ...
    wrong = KeySet(
        directory=tmp_path / "wrong_backend",
        backend="ckks",
        num_blocks=model.graph.num_blocks,
        profile="max_poly_degree=15",
    )
    with pytest.raises(ValueError, match="key/backend mismatch"):
        run_encrypted(model.graph, [[1] * 8], keys=wrong, backend="tfhe")
...
def test_predict_without_keys_uses_all_in_one(monkeypatch):
    ...
    def _fake(graph, int_inputs, *, keys=None, **kw):
        seen["keys"] = keys
